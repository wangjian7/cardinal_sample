...
'=====================================================================================
'==============================[ Integration Step ]============================================
' Integration Document Step 1a - Include Cardinal Widget Script
'=====================================================================================
--->
...
'=====================================================================================
'==============================[ Integration Step ]============================================
' Integration Document Step 1b - Create a function that initializes and runs the widget when the form is submitted
'=====================================================================================
--->
	<script type='text/javascript'>
		function runCardinalWidget(event){
			// Cross browser way to stop the form from submitting
			event.preventDefault ? event.preventDefault() : event.returnValue = false;
			try{
				// Create instance of widget
				var client = CentinelWidget.CreateModalWidget({
					LookupUrl : '<%= CENTINEL_LOOKUP_URL %>',
					NotificationUrl : '<%= MODAL_URL %>',
					TermUrl : '<%= CENTINEL_TERM_URL%>',
					ErrorCallback : handleError,
					// LoadingDisabled : true // Optional Field - This field will disable the normal loading window. An authentication screen will only be displayed if a card is enrolled. In the event a card is not enrolled, the widget will redirect to the term url.
				});
				
				// Add data on this page that needs to be passed to the lookup page. IE: Card Number, Card Expiry, ext
				/*
					Note: In our sample code we need to pass the entire form, but in a normal integration the majority of this data would live in the session and would not need to be passed this way.
				*/
				...
				client.AddByName("category_code", "category_code");
				client.AddByName("giftcard_amount", "giftcard_amount");
				...
				client.AddByName("product_code", "product_code");
				client.AddByName("txn_mode", "txn_mode");
				
				client.AddByName("email_address", "email_address");
				client.AddByName("customer_id", "customer_id");
				...
				console.log(e);
				// An Exception has occurred - submit the form as normal
...
//=====================================================================================
//==============================[ Integration Step ]============================================
// Integration Document Step 1c - Create error handlers defined in step 1b and add any additional functionality required to handle an error for
// your integration. 
//=====================================================================================
-->
		// Unified error handler - all errors will be handed to a single function that should be able to handle each type of error. An object will be passed back to this function giving details on the error encountered and any additional data associated with it.
		function handleError(errorData){
			console.log("Error recived: ", errorData);
			if(errorData.type === 'Ajax'){
				console.error('Ajax error detected', errorData.data);
			} else if(errorData.type === 'Lookup'){
				console.error('Lookup error detected', errorData.data);
			}  else if(errorData.type === 'Prompt'){
				console.error('Prompt error detected', errorData.data);
			}  else if(errorData.type === 'Resize'){
				console.error('Resize error detected', errorData.data);
			} else if(errorData.type === 'Animation'){
				console.error('Animation error detected', errorData.data);
			} else if(errorData.type === 'General'){
				console.error('General error detected', errorData.data);
			}  else {
				console.error('unknown type', errorData);
			}
		}
...
//=====================================================================================
//==============================[ Integration Step ]============================================
// Integration Document Step 1d - Attach a submit listener to intercept the form submission and start the widget.
//=====================================================================================
-->
			...
			if(form.attachEvent){
				// IE
				form.attachEvent('submit', runCardinalWidget);
			} else {
				// Majority of modern browsers
				form.addEventListener("submit", runCardinalWidget);
			}
			} // end onLoad
		</script>
...
<%@ include file="ccMenu.jsp" %>
		...
		<form id="frm" name="frm" method=post action="ccOrderComplete.jsp">
