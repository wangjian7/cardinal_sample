...
<html>
	<head>
		...
		//=====================================================================================
		%>
		<link rel="stylesheet" type="text/css" href="includes/css/customModal.css" />
		...
		<style>
			#renderAuthScreen{display:none; margin-top:5px; border:10px solid #A10001; padding:10px; border-radius:10px; box-shadow:3px 3px 10px 0 #777777;width:450px; position:absolute;top:-200px;z-index:9999;background-color:#fff;}
		...
		<script type="text/javascript">
				...
				try {
					...
					if (txn_type == "C") {
						...
						*/
						...
						client.ModalHeight = "50";												// Starting height for the widget element - used in both MODAL and INLINE - Default: 450
						client.ModalWidth = "225";												// Starting height for the widget element - used in both MODAL and INLINE - Default: 50
						...
						*/
						...
						client.AfterCloseCleanup = customClose;		
						client.BeforeResize = customBeforeResize;
						/*
						...
						*/
						client.EasingPackages = $.easing;									// Load the easing packages we have in the local jQuery from the included jQuery Plugin
						client.EasingOutType = "easeOutBounce";						// Easing package to use. Default: swing
						client.EasingInType = "easeInBounce";							// Easing package to use. Default: swing
						client.ResizeAnimationLength = 1000;
						client.LoadingImg = "includes/imgs/red-loading.gif"  			// Designate a loading Icon - If one is not defined then the Widget will not use one - Automatically positioned in center of ModalWindow element
						client.ExitImg = "includes/imgs/red-close-icon.png"			// Designate an exit Icon - If one is not defined then the Widget will not use one - Use CSS to position

						
						/*
						///////////////////// [ MODAL CONFIGURATIONS ] ////////////////////////
						Specific Configurations only to the modal style
						*/
						client.MaskColor = '#FFF';											// MODAL ONLY - Color override for the Mask Overlay on the Modal. Default: ##CCCCCC; Note: Double hash is used here since we're inside a cfoutput						
						...
						*/
						...
						var modalHeight = (winH/2) - 50;						// Window Height - ModalHeight intialized above
						var modalWidth = (winW/2) - 225; 				// Window Hidth - ModalWidth intialized above
						
						client.ShowModalAnimation = {'target':'#renderAuthScreen', 'css' : {'left':modalWidth, 'top':'-200px'}, 'options' : {'duration':500, 'queue':false, 'complete':function(){
								$(this).css('display','block');
								$(this).animate({'top':modalHeight},{'duration':250, 'queue':false});
							}
						}};
						...
						*/
						...
						client.AddByValue("OverrideLayout", "Layout2");
			...
			function customClose(){
				$('#CentinelIssuerLogo').attr('src','').hide();
				$('#CentinelProgramLogo').attr('src','').hide();
				$('#renderAuthScreen').css('padding-top','75px');
				$('.txtMsg').show();
			}
			
			function customBeforeResize(){
				var client = CentinelWidget.CentinelGetInstance();
				var lookup = client.GetLookupPayload();
				var ajaxResponse = client.GetAjaxResponse();
				if(lookup !== undefined && ajaxResponse !== undefined){
					if(lookup.isSecureCodePlus === true && ajaxResponse.AuthenticationPath === 'ENROLLED'){
						$('#CentinelIssuerLogo').attr('src',lookup.IssuerLogoUrl).show();
						$('#CentinelProgramLogo').attr('src',lookup.ProgamLogo).show();
						$('#renderAuthScreen').css('padding-top','10px');
						$('.txtMsg').show();
					} else {
						$('.txtMsg').hide();
					}
...
<tr>
	...
	<td>&nbsp;</td><td></td>
			...
			<div id="renderAuthScreen" class="logo">
			<div id="someMerchantContent">
				<ul id='extraContentToDisplay'>
					<li>
						<img src='' alt='Secure Code by MasterCard' id='CentinelProgramLogo' border='0' class='right' />
						<div class='clear'></div>
					</li>
