...
<html>
	<head>
		...
		<style>
			#renderAuthScreen{display:none; margin-top:5px; border:10px solid #445BA5; padding:10px; border-radius:10px; box-shadow:3px 3px 10px 0 #777777;width:450px; position:relative;}
			#authScreenGoesHere{margin-bottom:15px;}
			#loadingImg{vertical-align: middle;}	
			
			#someMerchantContent{width:450px;margin-bottom:10px;display:none;}
			#extraContentLoadingIcon{margin:0 auto; display:block;}
			#extraContentToDisplay{list-style:none; margin:0;padding:0;}
			#extraContentToDisplay li{margin:0;}
			#extraContentToDisplay li + li{margin-top:10px;}
			#extraContentToDisplay li p{margin:0;}
			#extraContentToDisplay li p + p{margin-top:10px;}
			
			#detailsCnt{border:1px sold black;padding-left:3px;display:none;overflow:hidden;}
			
			#CentinelProgramLogo{margin:0 auto; display:none;position:absolute; top:-40px; left:325px;}
			#CentinelIssuerLogo{margin:0 auto; display:none;position:absolute; top:-40px; left:300px;}

			.detailsList{list-style:none;margin:0;padding:0;}
			.detailsList li{position:absolute;top:-20px;}
			.detail{display:inline-block;}
			.detail-hding{font-weight:bold;display:inline-block;}
			.top-hding{font-weight:bold; font-size: 12px;}
			...
			.bounceIt-mid{
				-webkit-animation-name: bounce-mid; 
				-webkit-animation-duration: 1000ms;
				
				-moz-animation-name: bounce-mid; 
				-moz-animation-duration: 1000ms; 
				
				-animation-name: bounce-mid; 
				-animation-duration: 1000ms; 
			}
			...
			.bounceIt-bottom{
				...
				-moz-animation-name: bounce-bottom; 
				-moz-animation-duration: 1000ms; 
			...
			@-webkit-keyframes bounce-heading{
				0% {top:100px;}
				25% {top:80px;}
				50% {top:100px;}
				75% {top:90px;}
			...
			.bounceIt-heading{
				-webkit-animation-name: bounce-heading; 
				-webkit-animation-duration: 1000ms; 
				
				-moz-animation-name: bounce-heading; 
				-moz-animation-duration: 1000ms; 
		...
		<script type="text/javascript">
			...
			// This custom trigger is intended for demo purposes only and is not recommended for production use	
			var isRunning = 0;
			...
			$(function(){
				...
				$("#cvv").keyup(function(){
					var len = $("#cvv").val();
					if( len.length > 2 && isRunning == 0 ){
						isRunning = 1;
						runCentinelClient('<%= CENTINEL_TERM_URL %>', '<%= MODAL_URL %>', 'ccAjaxLookup.jsp', '<%= WIDGET_TIMEOUT %>')
					}
				});
			...
			function runCentinelClient(termUrl, modalUrl, generatePayloadUrl, timeout ) {
				...
				try {
					...
					var txn_type = $("[name=txn_type]").val();
					if (txn_type == "C") {
						/*
						///////////////////// [ URL DEFINITIONS ] ////////////////////////
						...
						*/
						client.RequestType = "inline";								// Use the modal style. Default: modal
						client.MessageType = "cmpi_lookup";								// If ADS - will complete an ADS transaction
						
						client.ModalWindow = "renderAuthScreen"; 					// Parent window 3DS iframe - When using Modal style one does not need to be designated. When using inline method one will need to be specified
						
						client.MerchantData = "here is some merchant data";		// MerchantData must be specified here

						/*
						///////////////////// [ ANIMATION HOOKS ] ////////////////////////
						Animation Hooks let you execute code before or after certain events. This allows for any kind of setup or clean up actions
						that may be needed to deal with different kind of events. Examples: If the merchant site would like to integrate a INLINE
						integration method where the Authentication screen is not shown until it is loaded. They may want to control the loading
						Icon themselves and display it in a completely different position then where the Authentication screen will appear. In this
						case the use of BeforeRenderSetup - to display it loading icon - and BeforeResize to destory the icon may be used.
						*/
						client.BeforeRenderSetup = CustomBeforeRenderSetup;									// Hook that executes before any rendering occurs
						client.BeforeResize = CustomBeforeResize;			// Hook that executes before any resizing is completed
						client.AfterCloseCleanup = CustomAfterCloseCleanup;									// Hook that executes after the widget closes and removes its elements from the DOM
						...
						*/
						...
						Specific Configurations only to the inline style
						*/
						client.ExtraContentId = "someMerchantContent";												// INLINE ONLY - Extra content that will be displayed above or below the ModalWindow element - Element must have a set height already or it will cause issues when resizing
						client.PlaceHolderDivId = "authScreenGoesHere"; 				// INLINE ONLY - Element to render inline content inside of. This should be the parent element of all content including the ModalWindow and ExtraContentId
						client.LoadBeforeResize = false;										// INLINE ONLY - Determines if the widget should appear before or after the Authentication screen has loaded. false = Show before loaded; true = Show after loaded; Default: false
						...
						*/
						...
						<!--- We can override the layout to our desired layout as we need --->
						client.AddByValue("OverrideLayout", "Layout2");					// Merchant override of the SecureCode+ layout to render.
			...
			function closeAuthenticationWindow(){
				...
				CustomAfterCloseCleanup();
			}
			
			function CustomAfterCloseCleanup(){
				$('#detailsCnt').css({'display':'none'});
				$('.detailsList li').css({'top':'-20px'}).removeClass('bounceIt-bottom').removeClass('bounceIt-mid').removeClass('bounceIt-top').children('span').removeClass('rockIt');
				$('#someMerchantContent').css({'margin-bottom':'10px'});
				$('#CentinelIssuerLogo,#CentinelProgramLogo').css({'top':'-40px'}).removeClass('bounceIt-bottom').removeClass('bounceIt-heading');
				
				$('input[type=submit]').removeAttr('disabled');
				isRunning = 0;
			}
			
			// Custom Animiation handler to show our own loaoding img
			function CustomBeforeRenderSetup(){
				var img = "<img src='includes/imgs/loading.gif' alt='Loading' id='loadingImg' />";
				$("#cvv").parent("td").append(img);
			}
			
			// Custom Animation handler
			function CustomBeforeResize(size){
				$('#loadingImg').remove();	
			
				var client = CentinelWidget.CentinelGetInstance();
				
				var lookupPayload = client.GetLookupPayload();
				var ajaxPayload = client.GetAjaxResponse();
				
				if(lookupPayload.isSecureCodePlus === true && ajaxPayload.AuthenticationPath !== "ADS"){
						$('#someMerchantContent').css({'margin-bottom':'100px'});
						
						var totalHeight = size.height + 175;
						var totalWidth = size.width + 200;
										
						var animation = {
							'target':'#'+client.ModalWindow,
							'css' : {'height': totalHeight+'px','width': totalWidth+'px'},
							'options' : {'duration':200, 'complete': function(){
									displayTxnDetails(lookupPayload,size)
								}
							}			
						};
						
						client.InlineContainerResizeAnimation = animation;
				}
			}
			
			function displayTxnDetails(payload,size){
			
				console.log(payload);
				
				$('#CentinelIssuerLogo').attr('src',payload.IssuerLogoUrl).show();
				$('#CentinelProgramLogo').attr('src',payload.ProgamLogo).show();
				
				$('.merchNameLbl').text(payload.MerchantNameLabel+':').next('span').text(payload.MerchantName);
				$('.amountLbl').text(payload.FormatAmountLabel+':').next('span').text(payload.FormatAmount);
				$('.maskedPanLbl').text(payload.MaskedPanLabel+':').next('span').text(payload.MaskedPan);
				
				$('#detailsCnt').css({'width':(size.width+200)+'px','display':'block'});
					
				$('.bottom').animate({'top':'+=180px'},{'complete':function(){
						$(this).addClass('bounceIt-bottom');
						$('.mid').animate({'top':'+=160px'},{'complete':function(){
								$(this).addClass('bounceIt-mid');
								$('.top').animate({'top':'+=140px'},{'complete':function(){
										$(this).addClass('bounceIt-top');
										$('.heading').animate({'top':'+=120px'},{'complete':function(){
												$(this).addClass('bounceIt-heading');
												$('#CentinelIssuerLogo').animate({'top':'+=200px'},{'complete':function(){
														$(this).addClass('bounceIt-bottom ');
														$('#CentinelProgramLogo').animate({'top':'+=140px'},{'complete':function(){
															$(this).addClass('bounceIt-heading');
															}
														});
													}
...
<tr>
	...
	<td>&nbsp;</td><td></td>
			...
			<div id="renderAuthScreen" class="left">
				<div id="detailsCnt" class="left">
					<img src='' alt='Secure Code by MasterCard' id='CentinelProgramLogo' border='0' class='left' />
					<img border="0" class="right" id="CentinelIssuerLogo" src="">
					<ul class="detailsList">
						<li class="heading"><span class="detail-hding top-hding">Transaction Details:</span></li>
						<li class="top"><span class="detail-hding merchNameLbl"></span> <span class="detail merchName"></span></li>
						<li class="mid"><span class="detail-hding amountLbl"></span> <span class="detail amount"></span></li>
						<li class="bottom"><span class="detail-hding maskedPanLbl"></span> <span class="detail maskePan"></span></li>
					</ul>
				</div>
				<div id="someMerchantContent">
					<ul id='extraContentToDisplay'>
						<li>
							<img src='includes/imgs/some-merchant-logo.gif' alt='Merchants Logo' id='myConsumerLogo' border='0' class='right' />
							<div class='clear'></div>
						</li>
						<li class="txtMsg"><p>Thank you for previously enrolling in MasterCard SecureCode, the safest way to pay on the Internet&reg;. Enter your SecureCode PIN below to confirm your identity and protect your transaction.</p></li>
					</ul>
				</div>
				<div id="authScreenGoesHere" class="left"></div>
				<div class="clear"></div>
			</div>
		</form>
