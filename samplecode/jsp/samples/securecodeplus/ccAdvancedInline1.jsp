...
<html>
	<head>
		...
		<style>
			...
			#CentinelProgramLogo{margin:0 auto; display:none;}
			#CentinelIssuerLogo{margin:0 auto; display:none;}
		...
		<script type="text/javascript">
				...
				try {
					...
					if (txn_type == "C") {
						...
						*/
						...
						Animiation Effects - [ Easing ] - jQuery comes with the ability to define and use custom animation effects. You can define your
						own and reference them here or use premade effects from other plugins. One of the most popular easing jQuery plugins can be
						found here: http://gsgd.co.uk/sandbox/jquery/easing/
						
						jQuery comes packages with 2 of its own easing styles: [swing] and [linear] 
						
						When using easing plugins - please be sure to include the JS file
						*/
						client.EasingPackages = $.easing;
						client.EasingOutType = "linear";						// Easing package to use. Default: swing
						client.EasingInType = "easeInBounce";						// Easing package to use. Default: swing
			...
			// Custom Animation handler to remove our custom loading image.
			function CustomBeforeResize(){
				var client = CentinelWidget.CentinelGetInstance();
				// Get lookup payload. We have extra details like issuer logo and program logo in this element
				var lookupPayload = client.GetLookupPayload();
				var ajaxPayload = client.GetAjaxResponse();
				// remove the loading icon - we need to call this because our loading icon is custom, put in from the CustomBeforeRenderSetup()
				$('#loadingImg').remove();	
				$('#validate').remove();
				// We only want to do this on MasterCards..				
				console.log(lookupPayload);
				if(lookupPayload.isSecureCodePlus){
					// custom text for ADS
					if(ajaxPayload.AuthenticationPath === "ADS"){
						var contentToRender = "<p>MasterCard SecureCode is a free security service that guards against unauthorized use of your existing MasterCard when you shop online at participating merchants like CompSource.</p><p>The MasterCard SecureCode program works with your existing MasterCard account, requires no download, and is easy to activate and to use.</p><p>Activate your card now and enjoy added peace of mind when shopping.</p>";
						$('.txtMsg').html(contentToRender);
						$('#myConsumerLogo').parent('li').hide();
					} else {
						// change src of image to the url we now have
						$('#CentinelIssuerLogo').attr('src',lookupPayload.IssuerLogoUrl).show();
						$('#CentinelProgramLogo').attr('src',lookupPayload.ProgamLogo).show();
					}					
					
				} else {
					$('#CentinelIssuerLogo').hide();
				}
